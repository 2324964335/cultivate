...
//import '../Exam/fault_sliver_self_controll.dart';
//import '../Exam/question_change.dart';
//import '../Exam/question_page.dart';
import 'QuestionPage.dart';
import 'QuestionSliverControllProvider.dart';
import 'QuestionProvider.dart';

class QuestionPageChangeMiddle extends StatefulWidget {
  QuestionSliverControllProvider controll;
  QuestionPageChangeMiddle({this.controll});

  @override
  State<StatefulWidget> createState() {
    // TODO: implement createState
    return QuestionPageChangeMiddleState();
  }
}

class QuestionPageChangeMiddleState extends State<QuestionPageChangeMiddle> {
...
class SliverPageSelfState extends State<SliverPageSelf>
  ...
  void initState() {
    super.initState();
    leftOffsetDistance = -ScreenUtil().setWidth(750);
  ...
  @override
  Widget build(BuildContext context) {
    Provider.of<QuestionSliverControllProvider>(context).self == SliverSelf.next;
    return _mainWidget();
  }
            ...
            child: Transform.translate(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  boxShadow: (leftOffsetDistance != -ScreenUtil().setWidth(750) ) ? [
                    BoxShadow(
  ...
  void _onUpdate(DragUpdateDetails details) {
    ...
    if (currentDrag >= 0) {
      //向右滑动
      if (Provider.of<QuestionProvider>(context, listen: false).current == 0) {
        //如果当前图为第一张则不让右滑
        currentDrag = -1;
        return;
      }
      ...
      //向左滑动
      if (Provider.of<QuestionProvider>(context, listen: false).current ==
          Provider.of<QuestionProvider>(context, listen: false).getQuestionList.length - 1) {
        //如果当前图为第一张则不让右滑
        currentDrag = 1;
        print(
  ...
  void centerEndSliver(bool isShow) {
    if (isShow) {
      ...
      animalController2.forward().then((_) {
        Provider.of<QuestionProvider>(context, listen: false).forward();
...
//      print('回收');
      ...
      animalController1.forward().then((_) {
        ...
        Provider.of<QuestionProvider>(context, listen: false).previous();
        leftOffsetDistance = -ScreenUtil().setWidth(750);
      });
    }
  }
